...
const CountersTableRow = ({ description, battleResult, clientData, opponent }: CountersTableRowProps) => {
  ...
  let battleUrl = undefined;
  if (opponent !== undefined) {
    const c = PlayerCreature.new();
    PlayerCreature.statefulSetSpecies(c, battleResult.playerSpecies);
    c.level = battleResult.playerLevel;
    c.fast = battleResult.playerFastMove;
        ...
        <>
          <div className='pt-2 md:pb-2 px-2 md:px-0 border-t flex flex-col md:flex-row border-theme4 divide-y md:divide-x md:divide-y-0 divide-theme4 gap-y-2'>
            <div className={openCategoryStyle}>
              ...
              {battleResult.playerSpecies.shadow && <div className='text-xs text-center italic'>Shadow Pokémon deal and take 20% more damage!</div>}
            </div>
            <div className={openCategoryStyle}>
              <h3 className='mb-2 text-lg flex justify-between items-center'>
                <span>Moves</span>
          ...
          {battleUrl !== undefined && <div className='p-2 border-t border-theme4 flex justify-end gap-2'>
            <Link
              href={battleUrl + '&solve_for_moveset=true'}
              className={cn(buttonVariants(), 'py-1 text-sm')}
            >
              View all moveset results
            </Link>
            <Link
              href={battleUrl}
              className={cn(buttonVariants({ variant: 'highlight' }), 'hover:bg-theme1 py-1 text-sm')}
            >
              Customize battle
            </Link>
          </div>}
