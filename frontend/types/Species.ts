...
export const Species = {
  ...
  getStats(species: Species, level: number, ivs: Stats3<number>): Stats3<number> {
  ...
  getCp(stats: Stats3<number>): number {
    return Math.max(Math.floor(0.1 * stats.atk * Math.sqrt(stats.def) * Math.sqrt(stats.hp)), 10);
  }
}